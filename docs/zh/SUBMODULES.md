...
本聚合仓库通过 **Git 子模块** 引用上游 Wi-Fi / Bluetooth Linux 驱动（或其它相关仓库）。**许可证以各子仓库根目录文件为准**，见根目录 `NOTICE` 与 [LICENSE_POLICY.md](LICENSE_POLICY.md)。
...
后续若使用 `git submodule add`，请优先使用上述路径，避免与已有文档和 CI 约定冲突。
...
- 在 `firmware/` 与 [FIRMWARE.md](FIRMWARE.md) 中声明 **固件与驱动的匹配关系**。
